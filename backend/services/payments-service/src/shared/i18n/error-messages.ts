...
export interface BilingualErrorMessage {
  ...
  am: string;
  code: string;
  category: 'FINANCIAL' | 'SECURITY' | 'VALIDATION' | 'SYSTEM' | 'COMPLIANCE';
}

export const ErrorMessages: Record<string, BilingualErrorMessage> = {
...
/**
 * Validates if an error code is in the whitelist
 * @param errorCode - The error code to validate
 * @returns True if the error code is valid
 */
function isValidErrorCodeInput(errorCode: string): boolean {
 ...
 */
function isValidLanguage(language: string): language is 'en' | 'am' {
  return language === 'en' || language === 'am';
}
...
export function getErrorMessage(
  ...
  // Security: Validate inputs to prevent object injection
  if (!isValidErrorCodeInput(errorCode) || !isValidLanguage(language)) {
    // Fallback error message for invalid inputs
    return language === 'am'
      ? 'ያልታወቀ ስህተት ተፈጥሯል።'
 ...
 */
export function isValidErrorCode(errorCode: string): boolean {
  // Security: Use whitelist validation instead of direct object access
  return isValidErrorCodeInput(errorCode);
}
