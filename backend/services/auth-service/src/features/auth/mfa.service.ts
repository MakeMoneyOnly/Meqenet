...
export class MfaService {
  ...
  async requestOtp(userId: string, channel: 'sms' | 'email'): Promise<{ message: string }> {
    // Rate limit OTP requests per user/channel
    await this.rateLimiter.checkRateLimit(userId, undefined, `mfa:${channel}`, 'REQUEST');
  ...
  async verifyOtp(userId: string, channel: 'sms' | 'email', code: string): Promise<{ verified: boolean }> {
    const key = `otp:${channel}:${userId}`;
