...
export function initializeOpenTelemetry(config?: OpenTelemetryConfig): void {
  try {
    ...
    } else if (config?.otelEndpoint || process.env.OTEL_EXPORTER_OTLP_ENDPOINT) {
      traceExporters.push(
        new OTLPTraceExporter({
          // grpc endpoint (e.g., http://otel-collector:4317)
          // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
          url: (config?.otelEndpoint || process.env.OTEL_EXPORTER_OTLP_ENDPOINT)!,
        })
      );
    }

    // Optional: logs exporter via OTLP if available
    const logExporter = (config?.otelEndpoint || process.env.OTEL_EXPORTER_OTLP_ENDPOINT)
      ? new OTLPLogExporter({
          // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
          url: (config?.otelEndpoint || process.env.OTEL_EXPORTER_OTLP_ENDPOINT)!,
        })
