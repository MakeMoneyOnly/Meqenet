...
export class SecretManagerService implements OnModuleInit {
  ...
  private previousPublicKey?: { kid: string; publicKey: string; rotatedAt?: string };
  ...
  async getJWKS(): Promise<JWKSResponse> {
    try {
      ...
      const graceDays = Number(this.configService.get<string>('JWKS_GRACE_DAYS', '7'));
      if (this.previousPublicKey && this.previousPublicKey.publicKey) {
        ...
        const withinGrace = rotatedAt
          ? (Date.now() - rotatedAt.getTime()) / (1000 * 60 * 60 * 24) <= graceDays
          : true;
        if (withinGrace) {
