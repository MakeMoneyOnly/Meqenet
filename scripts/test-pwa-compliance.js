...
class PWATester {
  ...
  async testManifest() {
    ...
    try {
      ...
      if (missingProps.length > 0) {
        this.addIssue(
          `Missing manifest properties: ${missingProps.join(', ')}`
        );
      } else {
      ...
      if (manifest.display === 'standalone') {
        ...
        this.addIssue(
          `Display mode is "${manifest.display}", recommended: "standalone"`
        );
      }
  ...
  async testServiceWorker() {
    ...
    try {
      ...
      if (fs.existsSync(appFile)) {
        ...
        if (
          appContent.includes('service-worker') ||
          appContent.includes('workbox')
        ) {
  ...
  async testOfflineCapability() {
    ...
    try {
      ...
      if (fs.existsSync(nextConfigPath)) {
        const configContent = fs.readFileSync(nextConfigPath, 'utf8');
        if (
          configContent.includes('runtimeCaching') ||
          configContent.includes('workbox')
        ) {
    ...
    try {
      ...
      try {
        ...
        if (fs.existsSync(reportPath)) {
          ...
          if (pwaScore >= 0.8) {
            this.addPass(
              `PWA Lighthouse score: ${(pwaScore * 100).toFixed(0)}%`
            );
          } else if (pwaScore >= 0.6) {
            this.addIssue(
              `PWA Lighthouse score is low: ${(pwaScore * 100).toFixed(0)}%`,
              'warning'
            );
          } else {
            this.addIssue(
              `PWA Lighthouse score is very low: ${(pwaScore * 100).toFixed(0)}%`
            );
          }
  ...
  async testBuildOutput() {
    ...
    try {
      ...
      if (fs.existsSync(publicBuildDir)) {
        const swFiles = fs
          .readdirSync(publicBuildDir)
          .filter(file => file.includes('sw.js') || file.includes('workbox'));
  ...
  calculateScore() {
    ...
    const weightedScore =
      (this.passed.length * 1.0 + this.warnings.length * 0.5) / totalTests;
...
`;
  ...
  async runAllTests() {
    ...
    this.log(
      `PWA testing completed with score: ${report.summary.overallScore}/100`
    );
