...
export class AuditLogger {
  ...
  static logTransaction(data: {
    ...
    console.log(
      `[AUDIT] Transaction ${data.transactionId} by user ${data.userId}: ${data.type} - ${data.amount}`,
    );
  }
  ...
  static logSecurityEvent(event: {
    ...
    // eslint-disable-next-line no-console
    console.log(
      `[SECURITY] ${event.type}${event.userId ? ` by user ${event.userId}` : ''} from ${event.ipAddress}`,
    );
  }
}
