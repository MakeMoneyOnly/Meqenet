 ...
 */
export function wrapContentWithScrollBlurItem(
  children: React.ReactNode,
): React.ReactNode {
  ...
  if (React.isValidElement<ElementProps>(children)) {
    ...
    if (
      children.type === ScrollBlurItem ||
      componentType?.displayName === 'ScrollBlurItem'
    ) {
      return children;
    }
    ...
    const isBlackCard =
      ...
      (className.includes('bg-black') || className.includes('rounded-3xl'));
