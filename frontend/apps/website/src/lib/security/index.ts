...
export class CryptoUtils {
  private static readonly ENCRYPTION_KEY = SecurityConfig.encryptionKey;
...
export class AuditLogger {
  ...
  static logTransaction(data: {
    ...
    // For now, log minimal audit information
    // eslint-disable-next-line no-console
    console.log(`[AUDIT] Transaction ${data.transactionId} by user ${data.userId}: ${data.type} - ${data.amount}`);
  }
  ...
  static logSecurityEvent(event: {
    ...
    // Security: Use secure logging service in production
    // For now, log minimal security event information
    // eslint-disable-next-line no-console
    console.log(`[SECURITY] ${event.type}${event.userId ? ` by user ${event.userId}` : ''} from ${event.ipAddress}`);
  }
}
