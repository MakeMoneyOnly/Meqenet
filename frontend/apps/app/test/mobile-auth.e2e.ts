...
describe('Mobile Authentication Flow', () => {
  ...
  describe('Login Functionality', () => {
    ...
    it('should handle successful login', async () => {
      ...
      await expect(
        element(by.text(`Welcome, ${testUser.firstName}`)),
      ).toBeVisible();
    });
    ...
    it('should show loading state during login', async () => {
      ...
      await expect(element(by.id('login-button'))).toBeNotVisible();
    });
  });
  ...
  describe('Registration Functionality', () => {
    it('should navigate to registration screen', async () => {
      ...
      await expect(element(by.id('email-input'))).toBeVisible();
      await expect(element(by.id('password-input'))).toBeVisible();
      await expect(element(by.id('confirmPassword-input'))).toBeVisible();
    ...
    it('should validate password requirements', async () => {
      ...
      await element(by.id('firstName-input')).typeText(newUser.firstName);
      await element(by.id('lastName-input')).typeText(newUser.lastName);
      await element(by.id('email-input')).typeText(newUser.email);
      await element(by.id('password-input')).typeText('weak');
      await element(by.id('confirmPassword-input')).typeText('weak');
      await element(by.id('terms-checkbox')).tap();
      await element(by.id('register-button')).tap();
      ...
      await expect(
        element(by.text('Password must contain uppercase letter')),
      ).toBeVisible();
    });
  ...
  describe('Accessibility', () => {
    it('should have proper accessibility labels', async () => {
      ...
      await expect(emailInput).toHaveLabel('Email address');
      await expect(passwordInput).toHaveLabel('Password');
      await expect(loginButton).toHaveLabel('Login to your account');
    });
  });
});
