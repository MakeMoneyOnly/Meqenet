...
This document provides comprehensive documentation for the internationalization (i18n) framework
implemented across the Meqenet platform, supporting both English and Amharic (አማርኛ) languages.
...
The frontend uses `react-i18next` for managing translations across both web (Next.js) and mobile
...
The backend uses a custom i18n module built on top of the `i18n` package for translating error
...
export class AuthController {
  ...
  async login(@Body() dto: LoginDto, @Req() req: I18nRequest) {
    ...
    // Success response
    return this.i18n.formatSuccess('success.loginSuccess', { userId: user.id }, { lang });

    // Error response
    throw new UnauthorizedException(this.i18n.formatError('invalidCredentials', 401, { lang }));
  }
}
```
...
```typescript
const errorMessage = this.i18n.getValidationError('email', 'required', { lang: 'am' });
// Output: "ኢሜይል ያስፈልጋል"
```
